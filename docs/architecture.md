...
`authpath-auditor` uses a small `src/` package with two layers:
...
The source reference `m14r41/PentestingEverything` influenced only the problem-space research. Implementation details are intentionally independent.
